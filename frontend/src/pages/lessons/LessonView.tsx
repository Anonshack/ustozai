...
export default function LessonView() {
  ...
  const { data: lesson } = useQuery<Lesson>({
    queryKey: ['lesson', id],
    queryFn: () => api.get(`/courses/lessons/${id}/`).then((r) => r.data),
  })
      ...
      {!quizMode ? (
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            {result.answers.map((a, i) => (
              <div key={i} className={`p-4 rounded-lg border ${a.is_correct ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                ...
                <p className="text-sm">
                  Sizning javobingiz: <span className={a.is_correct ? 'text-green-700 font-medium' : 'text-red-700 font-medium'}>{a.chosen_text}</span>
                </p>
                {!a.is_correct && (
                  <p className="text-sm text-green-700 mt-1">To'g'ri javob: <strong>{a.correct_choice}</strong></p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button onClick={() => setQuizMode(false)} className="btn-primary">
              Darsga qaytish
            </button>
          </div>
        </div>
